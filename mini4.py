...
def main():
    ...
    if c.fetchone()[0] == 0:
        print(f"{EMOJI['admin']} Thêm admin đầu tiên với ID: 7071414779, 7071414779 và 7071414779")
        c.execute("INSERT OR IGNORE INTO admins (chat_id) VALUES (?)", (7071414779,))
        c.execute("INSERT OR IGNORE INTO admins (chat_id) VALUES (?)", (7071414779,))
        c.execute("INSERT OR IGNORE INTO admins (chat_id) VALUES (?)", (7071414779,)) # Thêm ID admin mới
        conn.commit()
    conn.close()
